...
try{
  ...
  const css=await readFile('src/style.css','utf8')+await readFile('src/world-first.css','utf8');
  ...
  for(const [width,height] of [[600,634],[390,844],[328,730],[760,820],[844,390],[1440,900]]){
    ...
    if(width===600)assert.ok(bounds.bottom-bounds.top<=height*.32,'Compact mission selector must leave most of the Fold world visible');
  }
